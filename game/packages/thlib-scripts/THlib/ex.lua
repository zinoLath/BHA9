...
---平滑设置一个对象的变量
---@param obj lstg.GameObject @the obj you apply the smooth task
---@param valname string|number|function @索引或者一个变量设置函数
---@param y number @增量
---@param t number @持续时间
---@param mode function @参见移动模式
...
---@param starttime number @等待时间
---@param vmode number @MODE_SET, MODE_ADD, MODE_MUL，增量模式
function ex.AsyncSmoothSetValueTo(obj,...)
    local args = {...}
    task.New(obj, function()
        ex.SmoothSetValueTo(unpack(args))
    end)
end
----------------------------------------
---step
