...
function LoadImageRect(image, texname, l, r, b, t, ...)
    return LoadImage(image,texname, (l+r)/2, (b+t)/2, r-l, t-b,...)
end

function LoadImageGroup(prefix, texname, x, y, w, h, cols, rows, a, b, rect)
    local ret = {}
    for i = 0, cols * rows - 1 do
        table.insert(ret, prefix .. (i + 1))
        LoadImage(prefix .. (i + 1), texname, x + w * (i % cols), y + h * (int(i / cols)), w, h, a or 0, b or 0, rect or false)
    end
    return ret
end

function LoadImageGroupNamed(prefix, texname, x, y, w, h, cols, rows, a, b, rect)
    local ret = {}
    for i = 0, cols * rows - 1 do
        table.insert(ret, prefix[i + 1] or tostring(prefix.default) .. tostring(i+1))
        LoadImage(prefix[i + 1] or tostring(prefix.default) .. tostring(i+1), texname, x + w * (i % cols), y + h * (int(i / cols)), w, h, a or 0, b or 0, rect or false)
    end
    return ret
end
...
function LoadImageGroupNamedFromFile(texname, imgname, filename, mipmap, n, m, a, b, rect)
    LoadTexture(texname, filename, mipmap)
    local w, h = GetTextureSize(texname)
    return LoadImageGroupNamed(imgname, texname, 0, 0, w / n, h / m, n, m, a, b, rect)
end
