...
---@param f fun()
---@return thread
function task.NewNamed(target, name, f)
    ---@type thread[]?
    local tasks = rawget(target, field_named)
    if not tasks then
        tasks = {}
        ---@cast tasks -thread[]?, +thread[]
        rawset(target, field_named, tasks)
    end
    local co = create(f)
    tasks[name] = co
    return co
...
function task.Do(target)
    ...
    tasks = rawget(target, field_named)
    if tasks then
...
function task.ClearNum(target, reserve_current)
    if rawget(target,field) == nil then
        return
    end
...
---@param target any
---@param reserve_current boolean?
function task.ClearNamed(target, reserve_current)
    if rawget(target,field_named) == nil then
        return
    end
    if reserve_current then
        local co_current = co_stack[co_stack_n]
        ...
        if co_reserved then
            rawset(target, field_named, { co_reserved })
        else
            rawset(target, field_named, nil)
        end
    else
        rawset(target, field_named, nil)
    end
...
function task.Clear(target, reserve_current)
    task.ClearNamed(target,reserve_current)
    task.ClearNum(target,reserve_current)
end
